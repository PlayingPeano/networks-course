...
![5](images/image_5.png)
![6](images/image_6.png)
![7](images/image_7.png)
![8](images/image_8.png)
![9](images/image_9.png)
...
#### Демонстрация работы
![1](images/image_2_1.png)
![2](images/image_2_2.png)
![3](images/image_2_3.png)
![4](images/image_2_4.png)
![5](images/image_2_5.png)
...
#### Решение
Найдём время за которое отправится последний пакет. P-ый пакет будет ждать своей очереди, пока (P-1)-ый не пройдёт до первого маршрутизатора и не освободит дорогу, чтобы P-ый смог начать путь. Итого: ожидать нужно P-1 раз по L/R, а потом ещё пока преоодолеем весь путь до приёмника. Итого: $d_{\text{сквозная}} = (P - 1) \dfrac{L}{R} + N \dfrac{L}{R}$
...
#### Решение
Так как $R_1 -$ узкое место, то $T = \dfrac{5 * 8 * 2^{10}}{200} = 204.8$ с.

Если учитывать, что передача пакетная с размером L, то, не умаляя общности, в силу того, что первый канал $--$ самый медленный, нам нужно ко времени передачи добавить только время передачи последнего пакета по последним двум каналам. $T = \dfrac{F}{R_1} + \dfrac{L}{R_2} + \dfrac{L}{R_3}$, где $ F -$ это размер всего файла.
...
#### Решение
$P = \sum_{k = 12}^{60} C_{60}^k * 0.2^k * 0.8^{60 - k} = 0.551\dots$

Непонятно только зачем дана скорость передачи данных.
...
#### Решение
Воспользуемся формулой из 1 задачи. Пусть $P := X / S$, тогда $d = (3 + P - 1) * \dfrac{L}{R} = (2 + X / S) * \dfrac{80 + S}{R}$. Нужно минимизировать. Нехитро взяв производную, получим: $S_{opt} = \sqrt{40 * X}$ 
...
- При увеличении $\dfrac{L}{R}$ знаменатель $(1 - I)$ стремится к 1. Соответственно, тут прямая пропорциональность.
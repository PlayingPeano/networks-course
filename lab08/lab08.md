...
Тесты реализованы в `checksum_demo.py`:
- корректная полезная нагрузка проходит проверку;
- ошибка бита в данных детектируется;
- неверное значение контрольной суммы отклоняется.

Запуск:
`python3 checksum_demo.py`

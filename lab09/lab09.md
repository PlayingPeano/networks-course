...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - поррты -- это транспортный уровень. ICMP не для того, чтобы подключаться к процессу по номеру порта, он работает поверх IP, как вспомогательный протокол сетевого уровня.
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - тип:8, код:0.
   - ![3](images/image3.png)
   - аналогично по 2 байта на каждый.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - больше отправок, меняется TTL. В остальном -- так же всё.
   ![4](images/image4.png)
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - time-to-live exceeded: тип 11, код 0. IP-заголовок исходного проблемного пакета.
   ![5](images/image5.png)
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - они дошли до конечного хоста
   - TTL не обнулился на промежуточном роутере
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - нет, но некоторые хопы * * *.
   - нет, так как нет геометок

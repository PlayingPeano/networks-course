...
4. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ...
   ![8](images/image8.png)
   - на скриншоте видно, что нет ответов. (поле Answer RRs: 0).
...
1. Что такое база данных whois?
   - WHOIS — это публичные каталоги (и протокол запросов к ним), где для доменов и иногда IP/AS хранят регистрационные данные: кто регистрант, даты регистрации, статус, а для домена часто — список DNS-серверов (NS) и контакты регистратора. Доступ бывает через сайты регистраторов/регистратур или команду whois в терминале.
...
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - https://www.nic.ru/whois/, https://www.whois.com/whois/spbu.ru
